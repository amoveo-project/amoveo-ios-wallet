...
class PinCodeViewController: UIViewController {
    ...
    private let lineView = UIView(frame: CGRect.zero)
    ...
    override func viewDidLoad() {
        ...
        lineView.isHidden = DeviceType.isLaunchedOniPad
    ...
    func applyStyles() {
        lineView.backgroundColor = UIColor.rgb(0xffffff).withAlphaComponent(0.16)
    }
    
    func applyLayout() {
        let isWideScreen = DeviceType.isiPhone6OrMore
        let lineOffset = isWideScreen ? 85 : 45
        ...
        lineView.snp.makeConstraints { (make) in
            make.top.equalTo(submessageLabel.snp.bottom).offset(lineOffset)
...
extension PinCodeViewController {
    ...
    func present(with mode: PinCodeMode, delegate: PinCodeDismissDelegate?) -> PinCodeViewController? {
        ...
        locker.view.backgroundColor = UIColor.mainColor
        locker.mode = mode
