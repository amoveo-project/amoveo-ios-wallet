...
class EXACommon {
    ...
    static let hexadecimalRegex = "[0-9a-f]+"
...
#if TEST
        var path = "/Users/"
        let projectPath = AmoveoConstants.projectTestDataPath
    ...
    class func loadTestInfo(_ storageFileName: String) -> String? {
        ...
        do {
            let testInfo = try String(contentsOf: URL(fileURLWithPath: path), encoding: .utf8)
            return testInfo
        }
        catch {
            return nil
