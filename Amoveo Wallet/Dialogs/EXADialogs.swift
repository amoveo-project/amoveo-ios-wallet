...
class EXADialogs {
    ...
    class func showMessage(_ message: AWDialogMessage, dismissTimeout: Double? = nil, completionAction: (() -> Void)? = nil, dismissAction: (() -> Void)? = nil) {
    ...
    class func showError(_ error: Error, title: String = l10n(.commonError), buttonTitle: String = l10n(.commonOk), completionAction: (() -> Void)? = nil) {
        var message = error.localizedDescription
        var localTitle = title

        if error is AWError {
            message = (error as? AWError)?.description ?? ""
            localTitle = (error as? AWError)?.title ?? title
        }
    ...
    class func showEnterWalletPassword(completion: ((String) -> Void)? = nil, cancel: (() -> Void)? = nil) {
        if AppState.sharedInstance.settings.skippedPass {
            completion?("")
        } else {
