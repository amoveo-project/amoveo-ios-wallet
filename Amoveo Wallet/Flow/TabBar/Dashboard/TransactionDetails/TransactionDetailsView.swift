...
class TransactionDetailsView: UIView, UITableViewDataSource {
...
extension TransactionDetailsView: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        let hv = AmountHeaderView(width: tableView.bounds.size.width, title: viewModel?.txAttribute(by: TransactionAttribute.amount),
                color: UIColor.headerColor, textColor: UIColor.white)
        hv.txType = viewModel?.txType()
        hv.infoLabel.text = viewModel?.txAttribute(by: TransactionAttribute.alternativeAmount)
        hv.applyLayout()
